---
title: "Therapeutic Protocol Definition"
...
types_of_interest <- (crxg %>% 
    ...
    distinct() %>% count(cancer_type) %>% arrange(desc(n)) %>% filter(n > 40) %>% .$cancer_type)
...
I create a ggridges figure 

```{r}
order <- crxg_map_imputed %>% 
  gather(drug, response, -cosmic_id) %>% 
  group_by(drug) %>% 
...
crxg_map_imputed %>% 
  ...
  ggplot(aes(y = drug, x = response, fill = ..x..)) + 
  ...
  labs(x = "centered ln(IC50)",
       y = "Drug",
       fill = "Response Score",
       title = "Distribution of Response Scores by Drug",
       caption = "990 cell lines, median imputation") +
  ggsave("ridge_crxg.pdf", width = 6, height = 4)
```
